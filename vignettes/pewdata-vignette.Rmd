...
When run for the first time, the `pew_download` function will be ask for the information requested by Pew of those wishing to download its datasets: name, organization, phone, and email. 
After that information is input once, it will be saved in the `.Rprofile` and used automatically.

To change the input information, one may use the `options` function as below to input different values temporarily or hand-edit the `.Rprofile` file.
...
Alternately, one may set the argument `reset` to `TRUE` when running `pew_download` again, and the function will request (and store) the new information.

The `pew_download` function will then have access to the information it needs to pass on to Pew by default.  This means that researchers will not have to expose their info in their R scripts and that others reproducing their results later will be able to execute those R scripts without modification.  (Other researchers will, however, need to enter their own information into their own .Rprofiles, either by hand or via the `pew_download` command, a detail that should be noted in the reproducibility materials to avoid confusion.)
